...
class PWAConverter {
    ...
    generateServiceWorker() {
        ...
        const urlsToCache = [this.config.startUrl, `${this.config.startUrl}index.html`, `${this.config.startUrl}manifest.json`];
        this.files.allFiles.forEach(file => { urlsToCache.push(`${this.config.startUrl}${file.path}`); });
        if (this.config.enableOffline) { urlsToCache.push(`${this.config.startUrl}offline.html`); }
        const cacheStrategies = { 'cache-first': this.generateCacheFirstStrategy(), 'network-first': this.generateNetworkFirstStrategy(), 'stale-while-revalidate': this.generateStaleWhileRevalidateStrategy() };
        return `/**
...
self.addEventListener('fetch', e => { ${cacheStrategies[this.config.cacheStrategy]} });
...
function isNavigationRequest(r) { return r.mode === 'navigate' || (r.method === 'GET' && r.headers.get('accept').includes('text/html')); }`;
    ...
    generateCacheFirstStrategy() { return `if(e.request.method!=='GET')return;e.respondWith(caches.match(e.request).then(r=>{if(r)return r;return fetch(e.request).then(res=>{if(!res||res.status!==200||res.type!=='basic')return res;const c=res.clone();caches.open(CACHE_NAME).then(ch=>ch.put(e.request,c));return res}).catch(err=>{${this.config.enableOffline?`if(isNavigationRequest(e.request))return caches.match(OFFLINE_URL);`:''}throw err})}))`; } 
    generateNetworkFirstStrategy() { return `if(e.request.method!=='GET')return;e.respondWith(fetch(e.request).then(res=>{if(res.status===200){const c=res.clone();caches.open(CACHE_NAME).then(ch=>ch.put(e.request,c))}return res}).catch(()=>{return caches.match(e.request).then(r=>{if(r)return r;${this.config.enableOffline?`if(isNavigationRequest(e.request))return caches.match(OFFLINE_URL);`:''}})}))`; } 
    generateStaleWhileRevalidateStrategy() { return `if(e.request.method!=='GET')return;e.respondWith(caches.match(e.request).then(r=>{const f=fetch(e.request).then(res=>{if(res.status===200){const c=res.clone();caches.open(CACHE_NAME).then(ch=>ch.put(e.request,c))}return res}).catch(err=>{${this.config.enableOffline?`if(isNavigationRequest(e.request))return caches.match(OFFLINE_URL);`:''}throw err});return r||f}))`; } 
    generateNotificationHandlers() { return `self.addEventListener('push',e=>{const o={body:e.data?e.data.text():'New notification',icon:'${this.config.startUrl}icons/icon-192x192.png',badge:'${this.config.startUrl}icons/icon-72x72.png'};e.waitUntil(self.registration.showNotification('${this.config.name}',o))});self.addEventListener('notificationclick',e=>{e.notification.close();e.waitUntil(clients.openWindow('${this.config.startUrl}'))});`; } 
